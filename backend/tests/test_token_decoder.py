...
# RAuthy Token Decoder Tests


def test_verify_rauthy_access_token_expired_token(
    monkeypatch: pytest.MonkeyPatch,
):
    """Test that expired RAuthy tokens raise ValueError."""
    monkeypatch.setattr(
        "zimfarm_backend.api.token.OAUTH_ISSUER", "https://auth.example.com/auth/v1"
    )
    monkeypatch.setattr(
        "zimfarm_backend.api.token.OAUTH_AUDIENCE", "rauthy-client-id"
    )
...
def test_verify_rauthy_access_token_with_2fa_enabled_and_valid_amr(
    monkeypatch: pytest.MonkeyPatch,
):
    """Test successful RAuthy verification when 2FA is enabled and user has both factors."""
    monkeypatch.setattr(
        "zimfarm_backend.api.token.OAUTH_ISSUER", "https://auth.example.com/auth/v1"
    )
    monkeypatch.setattr(
...
def test_verify_rauthy_access_token_with_2fa_enabled_only_first_factor(
    monkeypatch: pytest.MonkeyPatch,
):
    """Test RAuthy verification fails when 2FA is enabled but only first factor is present."""
    monkeypatch.setattr(
        "zimfarm_backend.api.token.OAUTH_ISSUER", "https://auth.example.com/auth/v1"
    )
    monkeypatch.setattr(
